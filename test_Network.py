...
class TestNetwork(TestCase):
    def test_split_into_equal_sized_subnets(self):
        ...
        for cidr, correct, required_subnet_size in zip(
            cidrs, correct_subnets, required_subnet_sizes
        ):
    ...
    def test_calculate_first_ip_adr_classful(self):
        for network, ip in zip(
            CLASSFUL_NETWORKS.values(), ["1.0.0.0", "128.0.0.0", "192.0.0.0"]
        ):
            self.assertEqual(ip, network.first_ip)

    def test_calculate_lasts_ip_adr_classful(self):
        for network, ip in zip(
            CLASSFUL_NETWORKS.values(),
            ["1.255.255.255", "128.0.255.255", "192.0.0.255"],
        ):
            self.assertEqual(ip, network.last_ip)

    def test_calculate_first_ip_adr_classless(self):
        for network, ip in zip(
            CLASSLESS_NETWORKS.values(),
            ["192.168.0.0", "192.168.0.8", "10.0.0.0"],
        ):
            self.assertEqual(ip, network.first_ip)
    ...
    def test_calculate_lasts_ip_adr_classless(self):
        for network, ip in zip(
            CLASSLESS_NETWORKS.values(),
            ["192.168.0.255", "192.168.0.11", "10.0.15.255"],
        ):
            self.assertEqual(ip, network.last_ip)
